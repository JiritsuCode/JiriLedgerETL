...
export class RPCAPI {
  ...
  public async loadSourcesPaged(lastOffset: number) {
    ...
    try {
      let sources = await this.contract.methods.getSourcesPaged(this.pageSize, lastOffset).call();
      sources ||= [];
      return sources;
    } catch (error) {
      console.error("Error calling contract method:", error);
  ...
  public async loadVerifiers() {
    ...
    try {
      let verifiers = await this.contract.methods.getAllVerifierTypes().call();
      verifiers ||= [];
      return verifiers;
    } catch (error) {
      console.error("Error calling contract method:", error);
